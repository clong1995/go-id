...
func init() {
	base := []rune("0a¢⋼⋴⊘b⊛⊝JKLMNOn=☊pCDoEFqlmGH⊙IST;☈UVW⌂₫⋄yzA3B45ef@]g[hcRdk≎^ℯ1⊟2ij6789⊞PQ{x°}~X§Y⊠Z!∔$£₹ruv®wst⊡⊢⊣∅ℵα%≏↭()*៛,-ƒ¬±∉∩€β␏γδεζηθι™¥⊆⊇⊥⊤⊕␎␐¶©Ⅷ␁∪Ⅸℜ∐Ⅎ⅄ⅅⅆⅇⅈⅉ⊗πρσ∂∞τ␍␑υφξο∇∈∑∏∫√∝∧∨⊂Ⅳ␌⏛∆ⅤχψωΘΛΣ⚲Ψ⊃ΩⅡ␇⏞Ⅲλ⏦μνⅥ⏝⚶Ⅶ⏚⏟␉ⅾ⏼⏽␃ⅻⅼⅽⅿ❍✔⊜␊ⅶ␂ⅷ⅌⅍ⅸ⏜ⅹⅺⅬⅭⅫ␅Ⅾ␋⅊⅋ⅎ⅏ⅠⅩ⊚Ⅺ␄⏣␈ⅵⅯⅰ⏿␀␆ⅱⅲ⏡ⅳⅴ⊻⏠⏧")
	for i, v := range base {
		chars[i] = v
		maps[v] = int64(i)
	}
